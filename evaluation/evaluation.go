...
func (a Auditor) Evaluate(ctx context.Context, ex exercise.Exercise, source exercise.Context, reference, solution string) (Report, error) {
	...
	maxAttempts := a.MaxDecisionAttempts
	if maxAttempts < 1 {
		maxAttempts = 2
	}
	var attempts []result.Attempt
	var truthText, truthVerdict string
	var score int
	var assessment textguard.Assessment
	for decisionAttempt := 1; decisionAttempt <= maxAttempts; decisionAttempt++ {
		truthResponse, err := a.Client.Complete(ctx, api.Request{
			Model: a.Model, Instructions: instructions, Input: input, Effort: "high",
			Metadata: map[string]string{
				"task": "taocp-evaluation-truth", "exercise": fmt.Sprintf("%s.%d", ex.SectionID, ex.Number),
				"decision_attempt": fmt.Sprintf("%d", decisionAttempt),
			},
		})
		if err != nil {
			return partialReport(attempts, started), fmt.Errorf("criteria truth judge: %w", err)
		}
		attempts = append(attempts, makeAttempt("evaluation-truth", a.Model, truthResponse))
		...
		if decisionAttempt == maxAttempts {
			return partialReport(attempts, started), errors.New("criteria truth judge returned incomplete decision fields")
		}
	}
	...
	var auditText, auditVerdict string
	var auditAssessment textguard.Assessment
	for decisionAttempt := 1; decisionAttempt <= maxAttempts; decisionAttempt++ {
		auditResponse, err := a.Client.Complete(ctx, api.Request{
			Model: a.Model, Instructions: instructions, Input: input, Effort: "high",
			Metadata: map[string]string{
				"task": "taocp-evaluation-falsification", "exercise": fmt.Sprintf("%s.%d", ex.SectionID, ex.Number),
				"decision_attempt": fmt.Sprintf("%d", decisionAttempt),
			},
		})
		if err != nil {
			return partialReport(attempts, started), fmt.Errorf("adversarial audit judge: %w", err)
		}
		attempts = append(attempts, makeAttempt("evaluation-falsification", a.Model, auditResponse))
		...
		if decisionAttempt == maxAttempts {
			return partialReport(attempts, started), errors.New("adversarial audit judge returned incomplete decision fields")
		}
	}
...
func partialReport(attempts []result.Attempt, started time.Time) Report {
	return Report{
		Attempts: attempts, Metrics: result.BuildMetrics(attempts),
